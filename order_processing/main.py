...
def run_processing_example() -> None:
    ...
    user1_orders_data = [
        Order(id=101, order_type="A", amount=50.0, flag=False),  # Exported, Low Prio
    ...
    print(f"Processed Count (intended for DB update & succeeded): {processing_result.processed_count}")
    ...
    if processing_result.failed_orders:
        ...
        for order_id, reason in sorted(processing_result.failed_orders):
            print(f"  - Order ID: {order_id if order_id != -1 else 'N/A'}, Reason: {reason}")
    else:
        print("  None")
    ...
    try:
        ...
        if final_orders:
            ...
            for order in sorted(final_orders, key=lambda o: o.id):
                print(
                    f"  Order ID: {order.id:<3}, Type: {order.order_type:<1}, Amount: {order.amount:>6.2f}, "
