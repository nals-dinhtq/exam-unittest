...
class InMemoryDbService(DatabaseService):
    ...
    def __init__(self, initial_orders: Optional[Dict[int, List[Order]]] = None):
        ...
        self._all_orders_by_id = {
            order.id: order
            for user_orders in self._orders_by_user.values()
            for order in user_orders
        }
        log.info(
            f"InMemoryDbService initialized with {len(self._all_orders_by_id)} total orders.")

    def get_orders_by_user(self, user_id: int) -> List[Order]:
        log.info(f"DB: Getting orders for user {user_id}")
        # Simulate potential DB error
        # if user_id == 999:
        #     raise DatabaseException(f"Simulated DB error for user {user_id}")
        # Return copies to prevent direct modification of internal state elsewhere
        return [order for order in self._orders_by_user.get(user_id, [])]

    def update_order_statuses(
        self, updates: List[Tuple[int, str, str]]
    ) -> List[int]:
        log.info(
            f"DB: Attempting to update statuses for {len(updates)} orders.")
        failed_ids = []
        ...
        for order_id, status, priority in updates:
            ...
            if order_id == 110:  # Explicitly fail order 110 for demo consistency
                log.warning(
                    f"DB: Simulated explicit failure updating order {order_id}")
                failed_ids.append(order_id)
                continue
            ...
            if order_id in self._all_orders_by_id:
                ...
                log.debug(
                    f"DB: Updated order {order_id} to status={status}, priority={priority}")
            else:
        ...
        if not failed_ids:
            ...
            log.warning(
                f"DB: Bulk update completed with {len(failed_ids)} failures out of {len(updates)} attempts.")
        return failed_ids
