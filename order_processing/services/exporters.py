...
class CsvOrderExporter:
    ...
    @staticmethod
    def export_orders(orders: List[Order], user_id: int, timestamp: int) -> str:
        """
        ...
        try:
            with open(csv_file, "w", newline="", encoding="utf-8") as file_handle:
                ...
                # Write Header
                writer.writerow(["ID", "Type", "Amount", "Flag", "Status", "Priority", "Notes"])

                # Write Order Rows
                for order in orders:
                    # Use the already determined priority
                    priority_to_write = order.priority
                    notes = HIGH_VALUE_ORDERS if order.amount > HIGH_PRIORITY_THRESHOLD else ""
                    status_to_write = ORDER_STATUS_EXPORTED
            ...
            log.info(f"Successfully exported {len(successfully_exported_ids)} orders to {csv_file}")
            ...
            for order in orders:
                ...
                if order.id not in successfully_exported_ids:
                    order.status = ORDER_STATUS_EXPORT_FAILED
            raise CsvExportException(f"IOError exporting to {csv_file}: {e}") from e
